...
def spike_cleaning(ecog_data, z_thresh=5.0, mode='zero', return_spike_mask=False):
    event_epochs = ecog_data['signal']
    cleaned_epochs = event_epochs.copy()

    total_spikes = 0
    spike_mask_all = np.zeros_like(event_epochs, dtype=bool)

    for i in range(event_epochs.shape[0]):
        epoch = event_epochs[i]
        z = (epoch - np.mean(epoch)) / (np.std(epoch) + 1e-10)
        spike_mask = np.abs(z) > z_thresh
        num_spikes = np.sum(spike_mask)
        total_spikes += num_spikes
        spike_mask_all[i] = spike_mask

        if mode == 'zero':
            epoch[spike_mask] = 0.0
        elif mode == 'interp':
            epoch[spike_mask] = np.interp(np.flatnonzero(spike_mask),
                                          np.flatnonzero(~spike_mask),
                                          epoch[~spike_mask])
        else:
            raise ValueError("mod must be 'zero' or 'interp'")

        cleaned_epochs[i] = epoch

    cleaned_data = ecog_data.copy()
    cleaned_data['signal'] = cleaned_epochs

    print(f"Spike cleaning applied: z > {z_thresh} → mode: {mode}")
    print(f"Total spikes removed/interpolated: {total_spikes}")

    if return_spike_mask:
        return cleaned_data, spike_mask_all
    else:
        return cleaned_data
